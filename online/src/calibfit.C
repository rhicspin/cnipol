...
#define Debug 0
#define NOFIT 0     // 1 => no fitting. 0 => do fits.
...
//    	sprintf(str1,"CurC%d", Si+1);
//    	sprintf(str2,"Detector %d", Si+1);
//    	CurC[Si] = new TCanvas(str1,str2);
//    	CurC[Si] -> Divide(4,3);
        ...
        for (Int_t St=Si*12; St<Si*12+12; St++) {
            ...
            sprintf(title,"%s - strip %d", runid, St+1);
            hadc->SetTitle(title);           
        ...
        // Fit Only non-empty Hists
            ...
            if (hadc->GetEntries() > 30) {
             ...
             if (! NOFIT) {
                Int_t nfound = s->Search(hadc, 2, "goff", 0.5);
                if (nfound > 0) {
                 Float_t *xpeaks = s->GetPositionX();
	       	     fitf->SetParameter(1, xpeaks[nfound-1]);

	       		//fitf->SetParLimits(1, MaxPosition-2, MaxPosition+2);

	       		 hadc->Fit("fitf","","0",MaxPosition-10., MaxPosition+10.);
	       		//hadc->Fit("fitf","E","0",1, 256);
               //hadc -> Fit("gaus","E", "0",MaxPosition-10., MaxPosition+10.); // 20. 20.
//              hadc -> Fit("gaus","E", "0",0., 255.);
        ...
        // Draw +/- 7sigma region
        hadc -> GetXaxis()->SetRangeUser(0., 255.);
/*              hadc -> GetXaxis()-> 
              SetRangeUser( 
...
void calibfit::PlotResult()
{
	...
	gStyle->SetFillColor(kWhite);
    TCanvas *CurS3 = new TCanvas("CurS3","Summary",1);
