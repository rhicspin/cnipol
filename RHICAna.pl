...
printf " OnlineLogDir    =   $OnlineLogDir\n";
...
$frame3->Button(-text => "Browse", -command=>\&conflist)
    ->pack(-side=>'left',-anchor=>'w');
$frame3->Button(-text => "Online", -command=>\&onlineconfig)
    ->pack(-side=>'left',-anchor=>'w');
...
sub show_result {
    $Runn = sprintf("%04d.%03d",$fillnumber,$runnumber);
    $ENV{"RUN"} = $Runn;
    system("xterm -e tcsh -c \"paw -w 1 -b kumac/asymplot.kumac \" &");
...
sub scaler_banana {
    ...
    system("xterm  -e tcsh -c \"nice +20 paw -w 1 -b $BaseDir/kumac/cutq  \"& ");
}
...
("%04d.%03d",$fillnumber,$runnumber);
    $ENV{"RUN"} = $Runn;
    system("./kumac/lr_spinpat.pl $Runn");
    system("xterm  -e tcsh -c \"nice +20 paw -w 1 -b $BaseDir/kumac/lr_bunch  \" &");
}
...
sub pvector {
    $Runn = sprintf("%04d.%03d",$fillnumber,$runnumber);
    $ENV{"RUN"} = $Runn;
    system("./kumac/pvector.pl $Runn");
    system("xterm  -e tcsh -c \"nice +20 paw -w 1 -b $BaseDir/kumac/pvector.kumac  \" &");
}
...
sub onlineconfig(){
	...
	$Runn = sprintf("%04d.%03d",$fillname,$runname);
	open(DLOG,"$OnlineLogDir/$Runn.log");
	while ($line = <DLOG>){
	    chop($line); @words = split(/\s+/,$line);
