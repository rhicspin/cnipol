  ...
  long int * pointer ;
    ...
    while (flag==0) {
        ...
        case REC_READAT:
          ...
          // Count the number of read data records
          //ds
          nDataRecs++;
            ...
            for (int i=0; i< rec.header.len - sizeof(recordHeaderStruct);) {
                ...
                for (j=0; j<Nevent; j++) {
                    ...
                    if (fmod(float(Nread),float(THINOUT))==0) {
                        ...
                        if (root && dproc.SAVETREES.any()) {
                           root->AddChannelEvent(event);
                           if (dproc.SAVETREES.test(0)) root->FillTree();
                        }
