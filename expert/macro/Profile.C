...
Float_t 
PolarizationProfile(StructProfile profile, Int_t ndata, Int_t Color, Int_t DoFit){

  Int_t MStyle = DoFit ? 31 : 20;
  Color = DoFit ? 1 : Color;
  ...
  if (DoFit) Pol->Fit(f1,"same");


  return DoFit ? f1->GetChisquare()/f1->GetNDF() : 0 ;
...
Float_t 
MakePlot(StructProfile &profile, TPostScript * ps, Int_t k)
{
  ...
  if (k!=-1) Chi2NDF = PolarizationProfile(profile, profile.ndata-k, 2, 1);  C->Update();
  ...
  if (k==-1){
    ...
    cout << "Max rate=" << g->GetParameter(0) << endl;
...
Int_t DataLoop(StructProfile profile, TPostScript * ps){
  ...
  MakePlot(profile, ps, -1); 
  for (Int_t k=0; k<profile.ndata-1; k++)  profile.Chi2NDF[profile.ndata-1-k] = MakePlot(profile, ps, k);
